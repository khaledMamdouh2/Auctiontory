...
public class PartsBidControllerImpl implements PartsBidController {
    ...
    public boolean alreadyBid(int userId, int partProductId, int partAuctionId) {
        ...
        try {
            utx.begin();
            alreadyBid = partsBidDao.alreadyBid(userId, partProductId, partAuctionId);
            utx.commit();
        } catch (Exception e) {
            try {
                utx.rollback();
    ...
    public boolean bid(int userId, int partProductId, int partAuctionId, int bidAmount) throws AlreadyHighestBidderException, AuctionAlreadyClosedException {
        ...
        try {
            utx.begin();
            bid = partsBidDao.bid(userId, partProductId, partAuctionId, bidAmount);
            utx.commit();
    ...
    public List<UserPartBid> loadAll() {
        ...
        try {
            utx.begin();
            userPartBids = partsBidDao.loadAll();
            utx.commit();
        } catch (Exception e) {
            try {
                utx.rollback();
    ...
    public UserPartBid get(Serializable id) {
        ...
        try {
            utx.begin();
            userPartBid = partsBidDao.get(id);
            utx.commit();
        } catch (Exception e) {
            try {
                utx.rollback();
